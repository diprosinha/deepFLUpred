"""Command-line interface for FluMAPPER."""
...
from flumapper import __version__
from flumapper.constants import PATHOGENICITY_HOSTS, SEGMENTS
from flumapper.model_store import ModelStore
from flumapper.predict import predict_fasta
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="flumapper",
        description=(
            "FluMAPPER: an AI-based Molecular Analysis, Prediction and Profiling "
            "platform for influenza viruses. Identifies HA/NA segments, predicts "
            "HA (H1-H16) / NA (N1-N9) subtype, and -- for HA -- predicts HPAI/LPAI "
            "pathogenicity from the HA1/HA2 cleavage site."
        ),
    )
    parser.add_argument("--version", action="version", version=f"FluMAPPER {__version__}")
